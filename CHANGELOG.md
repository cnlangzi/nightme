...
- **CLI log surface (fix)**: the structured logger now tees every
  line to the persisted log file **and** stderr, so the terminal
  shows the same trace the file captures. Companion to the
  Feishu adapter's `logInbound` call wiring — `handleMessage`
  now emits one `feishu: incoming` line per user message (the
  function was defined but never called since commit `0a0e021`
  left the call edge as a stale comment). Combined, the CLI now
  shows both halves of every conversation:
  ...
  cover the stderr sink and the inbound log emission.
