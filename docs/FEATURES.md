...
| F-51 | **Slash Command / Service 分离重构** — 撤回 §1.3 / §1.4 违反：chatsession 不该有 GTW* 字段/方法/类型；slash command 提为 `internal/command/` 包（Commander 接口 + 各命令 SlashCommandFactory + RuntimeServices + SessionService / ReactionRouter 等 service 接口）；gtw 全部代码（含 state types、修复逻辑、reaction handler、debug scenarios）搬入 `internal/command/gtw/`；reaction 路由从 `ChatSession.SetActionHandler` / `HandleAction` 抽出到 `command.ReactionRouter` 接口（runtime 装配）；chatsession 删 `gtwContext`/`gtwDrafts` 字段 + 10 个 GTW-prefixed 方法 + 整 `gtw_state.go` / `gtw_accessors.go` 文件；删 5 个 type alias；新增 §1.4 第 4 层抽象「Service interface」 | [feat/F-51-slash-command-service-separation.md](./feat/F-51-slash-command-service-separation.md) + [`SPEC §0.15`](./SPEC.md#015-文档变更摘要v13x-f-51-slash-command--service-分离重构2026-08-06) + [`SPEC §0.16`](./SPEC.md#016-文档变更摘要v13x-f-51-4-批次迁移计划2026-08-06) | v1.3.x | 🚧 B0-B3 已落地（gtw 子集）；B5+（其余 7 个 slash command）后续 PR |
