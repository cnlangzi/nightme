...
| F-46 | **teamflow `/gtw fix` 团队工作流 v1**（借鉴 [cnlangzi/gtw](https://github.com/cnlangzi/gtw) 两步确认 + 标签状态机思路，不参考其代码）— v1 仅 `/gtw fix <id>`（认领 issue + 建 worktree + 打 nightme/wip label）。`internal/gtw/` 包（10 文件，约 1800 LOC），零新增文件、零特殊路径、零新消息类型；worktree 放兄弟目录 `<repo>.nightme/<slug>/`；token 借 `gh`/`glab`；ChatSession 加 2 字段（gtwContext / gtwDrafts）+ 8 accessor；runtime startup 接 `gateway.RegisterGTW`；daemon 重启时 `RebuildContext` 从 cwd 隐式恢复。22 unit tests + 2 end-to-end (happy path + recovery)。架构侵入点：`ChatSession` 不变；reaction ingest + reaction 分支路由在 `feat/gtw-reaction` 分支单独交付 | (本 PR commit message 即设计入口；详见 PR #50) | v1.3.x | 🚧 PR #50 (review 中) |
