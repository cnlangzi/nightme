...
| F-43 | **`/close` graceful + `/new` ResumeID clear + per-entry list reply** — `KillAll` 走 bridge.Close graceful 路径（5s outer timeout）；dead entry 的 ResumeID 清空防止 `--resume <dead-id>` 复活；`✓/✗/•` 三种 emoji 的 per-agent 列表回复，4KB 字节 cap + ...and N more 截断（Feishu 限制）| [feat/F-43-close-new-graceful-and-reset.md](./feat/F-43-close-new-graceful-and-reset.md) | v1.3.x | 🚧 PR #44 已开（review 中）|
...
| F-45 | **Main-Chat 卡片 Footer (per-turn snapshot)**（兑现 F-44 §6.1 推迟）— `AgentSession` 自管 metadata（`Model` + `CompactionCount` + Prompt lifecycle）。**usage 不再跨 turn 累计**：bridge 报的 `ResultEvent.Usage` / `DoneEvent.Usage` 直接透传到 `SessionContext.Usage`,runtime 完全 pass-through。`OutboundMessage` 加 1 个 typed snapshot field `SessionContext *SessionContext`（不是 3 个分散字段），runtime 在 `newEventHandler` 一次性 stamp 到 4 个 main-chat Kind（`OutReply` / `OutResult` / `OutTaskCreate` / `OutTaskUpdate`）。`UsageInfo` 从 `gateway` 搬到 `agent` 包消除反向 import。Footer 格式 **F-52 后**（D 版 `「」` 包裹）：`🤖 claude · opus-4-5` + `💰:「 20.5k / 1.5k · 10.5% · $0.087 」`（`in` = 三个 input-side 字段之和；`X%` = bridge 算 (per-turn context-window %,Doc 1 公式),runtime 透传；`$cost` = API 透传 `total_cost_usd`,客户端不计算）。`OutInit` / `OutUsage` 仍是 silent drop（不变），footer 数据走 `SessionContext` 单独路径。详见 [`wip/usage.md`](./wip/usage.md) 重构清单 | [feat/F-45-session-footer.md](./feat/F-45-session-footer.md) | v1.3.x | 📝 设计阶段（doc-first）|
...
| F-51 | **Slash Command 分层** — `/cwd` `/use` `/close` `/new` `/watch` `/think` `/tools` `/gtw` 统一由 `internal/command/` 的 Commander / Registry / Factory 路由；需要 chat-session 状态的 Factory 直接使用 `*chatsession.Manager`，reaction 由 `ReactionRouter` 统一分发 | [SPEC](./SPEC.md) | v1.3.x | ✅ 已实现 |
...
| F-56 | **`/gtw push` 三分支流 + Agent 最小化 Prompt** — 把 push 拆成 3 个互斥分支（no-op / dirty→agent+push / clean→直推）；agent 职责从"commit + push + 报告"收窄到"按相关性拆 commit 到本地"（不 push、不汇报）；nightme 工程化接管 push / IM 卡片 / verification，git 状态是唯一 source of truth（不再读 agent prose）；PR #120 加的 HEAD-advance check 升级为 Branch 2 的 `verifyAgentCommitted`；`pushClean` / `pushDirty` 收编为 `programmaticPushWithRetry`；gtw README §1.1 emoji 字典加 `ℹ️`（no-op case）。动机：实测 case agent 报 `94c4a38` 但 HEAD unchanged at `94c4a3866268` —— prompt 给的"Reply with: <sha> <summary>" 是 LLM 撒谎的显式口子，删掉它 | [feat/F-56-gtw-push-three-branch.md](./feat/F-56-gtw-push-three-branch.md) | v1.3.x | 📝 设计阶段（doc-first）|
