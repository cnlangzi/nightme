...
| F-45 | **Main-Chat 卡片 Footer + AgentSession 累计 Token 持久化**（兑现 F-44 §6.1 推迟）— `AgentSession` 自管 metadata（`Model` + `cumulativeUsage UsageInfo` + mutex + dirty flag），EventInit 时 capture model，EventUsage 时累加，EventDone 时落盘 `agent_sessions.json`；**仅 `/new` 清零**（daemon 重启 / `/cwd` / `/use` / `/kill` 保留）。`OutboundMessage` 加 1 个 typed snapshot field `SessionContext *SessionContext`（不是 3 个分散字段），runtime 在 `newEventHandler` 一次性 stamp 到 4 个 main-chat Kind（`OutReply` / `OutResult` / `OutTaskCreate` / `OutTaskUpdate`）。`UsageInfo` 从 `gateway` 搬到 `agent` 包消除反向 import。Footer 格式 **F-52 后**（D 版 `「」` 包裹）：`🤖 claude · opus-4-5` + `💰:「 20.5k / 1.5k · 10.5% · $0.087 」`（`in` = 三个 input-side 字段之和；`X%` = API 报 ContextWindow 算占比；`$cost` = API 透传 `total_cost_usd`）。`OutInit` / `OutUsage` 仍是 silent drop（不变），footer 数据走 `SessionContext` 单独路径 | [feat/F-45-session-footer.md](./feat/F-45-session-footer.md) | v1.3.x | 📝 设计阶段（doc-first）|
