...
`sessionId` 就是 `session/new` 的返回值。`session/load` 读取 `~/.cursor/acp-sessions/<sessionId>/store.db`。第一次 `session/prompt` 结束之后这个文件才存在；只有 `meta.json` 时 load 返回 `-32602`，`error.data.message` 为 `Session "<id>" not found`，bridge 包成 `agent.ErrResumeUnhealthy` 交给 chat 层。不换另一个 session。

```
session/load {
  sessionId: <session/new 的 UUID>,
  cwd: <绝对路径>,
  mcpServers: []
}
```

响应不回带 `sessionId`。客户端继续用请求里的那个 id。历史以 `session/update` 回放，bridge 不转发到聊天。
