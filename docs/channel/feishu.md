...
- **ReplyInThreadAndChat (字段省略)** — `OutCard` (permission) / `OutCommandReply` / `OutCompaction`：用户首要看到的答案 / 不可漏看的权限请求 / slash 命令回应 / brief 进度 marker，必须 main chat 可见
- **ReplyInChat (顶级 Create)** — `OutReply` / `OutResult` / `OutTaskCreate` / `OutTaskUpdate` (F-44 follow-up)：每个 chunk / 每条 result / 任务 receipt card 独立成 top-level bubble，**不**锚定到 user 消息。理由：一旦同 turn 的 `OutToolStart` / `OutToolEnd`（用 `ReplyInThread`）在 user message 上建了 thread，**Feishu server 会把后续所有 `ReplyInBoth` 拉进 thread 抽屉，主 chat 看不到**（reply.go docstring 第 17-19 行明文）。`OutTask*` 的 cold-start card 通过 `SendCard` 走 `rootID=""` 强制 top-level Create；后续 `SetTaskList` 走的 `PatchMessage` 在 PATCH 时保留原 message 的 root_id 状态（顶级 Create 保持顶级）。Fallback 路径 230011/231003 也走 `ReplyInChat`（详见 §15.2）。
