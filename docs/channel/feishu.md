...
- **2026-08-21** - chatID 收口到 `SessionChatID` 纯函数（fix-feishu-chat-id）。三处 inbound 入口（`handleMessage` / `handleReactionCreated` / `handleCardAction` 全部子路径）统一走 `a.SessionChatID(source)`，不再各自取 SDK 字段。详见本节 §14.1 契约。**不做** `on_<hex>` 兼容 / 不做迁移 / 不做 `IsValidFeishuChatID` 格式校验 — SDK 给啥用啥，空就 drop。改动文件：`internal/channel/feishu/session_chatid.go`（新增）+ `internal/channel/feishu/session_chatid_test.go`（新增）+ `internal/channel/feishu/adapter.go`（三处入口改走 SessionChatID，删除 `extractReactionChatID` 公开契约语义）。
...
### 14.1 📌 chatID 契约:Fix-Feishu-Chat-ID (2026-08-21)

**入口唯一性**:Feishu adapter 的 chatID 提取,统一通过 `(*Adapter).SessionChatID(event SessionChatIDSource) string` 纯函数。三处 inbound 入口全部走它:
