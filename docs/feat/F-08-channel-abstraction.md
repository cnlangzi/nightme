...
**v1.3.x 扩展**:Channel 按 `OutboundKind` 自决 routing（Feishu 选 thread + 类型感知摘要，详见 [`F-37-tool-thread-routing.md`](./F-37-tool-thread-routing.md) + [`channel/feishu.md` §13.12](../channel/feishu.md)）。这是 "concrete stays concrete" 原则的具体落地：Gateway 只发 `OutboundMessage{Kind, ReplyTo, ...}`；Channel 看到 Kind 后自决 thread reply / receipt card / reaction。
...
| **v1.3.x: Put routing hint in `OutboundMessage.Meta`** | **Meta carries data only** (output / err / args), not routing hint. Channel reads Kind and decides routing itself (Feishu: thread reply for thinking/tool/compaction; receipt card for text/result/init/usage) |
...
| **v1.3.x: 自决按 OutboundKind 分流**（thread reply / receipt card / reaction / ...）— 自治范围内的渲染决策。例：Feishu 选 thinking/tool/compaction → thread reply（详见 [`F-37-tool-thread-routing.md`](./F-37-tool-thread-routing.md)）|
...
| OutThinking / OutToolStart / OutToolEnd 太多元素挤占 receipt card | **v1.3.x F-thread-route**: Feishu 把这些 Kind 路由到独立 thread reply（rootID = userMsgID），receipt card 收窄到只承载最终答复；详见 [`F-37-tool-thread-routing.md`](./F-37-tool-thread-routing.md)。其他 Channel 自决（折叠 section / 子节点 / drop）|
...
- **2026-08-04** — v1.3.x F-thread-route (SPEC §0.3): Channel 按 OutboundKind 自决 routing。Feishu 选 thread reply + 类型感知摘要(`OutThinking` / `OutToolStart` / `OutToolEnd` / `OutCompaction` → thread;`OutText` / `OutResult` / `OutInit` / `OutUsage` → receipt card;`OutMessageState` / `OutCard` → 不变)。§4 contract 表更新 —— Meta 只承载数据载荷,不承载 routing hint;Channel 看到 Kind 后自决。详见 [`F-37-tool-thread-routing.md`](./F-37-tool-thread-routing.md) + [`channel/feishu.md` §13.12](../channel/feishu.md)。
- **2026-08-05** — v1.3.x F-44 (SPEC §0.11): 简化 receipt card —— `OutReply` 拆出 receipt 改独立 `ReplyInThreadAndChat`;task receipt card 只剩 Tasks section;`OutInit` / `OutUsage` 改 **silent drop** (推迟 footer PR)。详见 [`F-44-outreply-independent-and-task-receipt.md`](./F-44-outreply-independent-and-task-receipt.md) + `SPEC §0.11`。
- **2026-08-05** — v1.3.x F-45 (SPEC §0.12): Main-chat 卡片 footer —— `OutboundMessage` 加 `SessionContext *SessionContext` typed snapshot, runtime 在 `newEventHandler` 一次性 stamp 到 4 个 main-chat Kind;`AgentSession` 自管 metadata(`Model` + `cumulativeUsage UsageInfo`),持久化到 `agent_sessions.json`,**仅 `/new` 清零**。`OutInit` / `OutUsage` 保持 silent drop(不变);footer 数据走 `SessionContext` 单独路径。详见 [`F-45-session-footer.md`](./F-45-session-footer.md) + `SPEC §0.12` + `channel/feishu.md §13.22`。
