# F-34: OutToolEnd + OutThinking → Thread Reply (with Type-Aware Summary)
...
type ToolEndEvent struct {
    ...
    // type-aware summaries (F-34 §2.3) without re-parsing the
...
case agent.EventText:
    ...
    if strings.HasPrefix(text, thinkingPrefix) {
        // F-34: thinking 走 thread，不再 fold 进 receipt
        return LogEntry{}, false
    }
    return LogEntry{Icon: "💬", Text: truncateForLog(text, perEntryMaxBytes), Kind: "reply"}, true

case agent.EventToolStart:
    // F-34: tool_start 走 thread
    return LogEntry{}, false

case agent.EventToolEnd:
    // F-34: tool_end 走 thread (类型感知摘要)
    return LogEntry{}, false

case agent.EventCompaction:
    // F-34: compaction 走 thread
    return LogEntry{}, false
```

**`Gateway.translate.go` 是否需要改**？

`OutThinking` 当前由 Gateway 在 `Translate` 里把 `[思考] ` 前缀剥掉再 emit。**F-34 之后**：
...
详见 [`docs/SPEC.md` §0.3](./SPEC.md) + [`docs/feat/F-34-tool-thread-routing.md`](./feat/F-34-tool-thread-routing.md) + [`docs/channel/feishu.md` §13.12](./channel/feishu.md)。
```
...
- **2026-08-04** — F-34 草案（Devin 拍板反转 §13.6 折叠方案）。Docs 落地（SPEC §0.3 + 本 doc + channel/feishu §13.12 + F-25 §3 收窄 + F-08 §4 自治路由例子 + CHANGELOG）。代码改动 backlog §3.1。
