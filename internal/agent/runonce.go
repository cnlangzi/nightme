...
// RunOnceDrain sends blocks on a live Agent and drains Events() until
// the turn ends. Used by structured bridges (claudecode / codex / pi
// / acp) — PTY has its own heuristic impl because it has no
...
// AgentSpec.Name() so users see "claude" not "claudecode.Agent".
