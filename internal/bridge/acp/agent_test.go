...
func TestAgent_Name(t *testing.T) {
	a := NewAgent("codex", "codex", []string{"--acp"})
...
func TestAgent_Detect(t *testing.T) {
	...
	if err := NewAgent("echo", "/bin/echo", nil).Detect(); err != nil {
		t.Fatalf("Detect(/bin/echo) = %v", err)
	}
	if err := NewAgent("missing", "/no/such/acp-agent", nil).Detect(); err == nil {
...
func TestAgent_Start_RequiresBinary(t *testing.T) {
	...
	_, err := NewAgent("missing", "/no/such/acp-binary", nil).Start(
