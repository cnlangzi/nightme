...
func TestStart_ResumeMultiTurnRespawn(t *testing.T) {
	...
	if !gotInit {
		t.Fatalf("phase 2 turn A: never saw AgentConnected — regression of test19 hang")
	}
...
// an AgentConnected with a non-empty SessionID, or until the deadline
...
func captureInitSessionID(t *testing.T, sess agent.AgentSession, timeout time.Duration) string {
	...
	for {
		...
		case <-deadline:
			t.Fatalf("captureInit: no AgentConnected within %s — claude hung (regression of test19 hang)", timeout)
		}
	}
}