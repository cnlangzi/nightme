...
// attachMethodHandler installs the Cursor-specific extension method
// handler (cursor/update_todos, cursor/create_plan, cursor/task,
// cursor/ask_question, cursor/generate_image). Tolerates a missing
// DriverHandle so the bridge returns the live *Agent either way —
// extension events just fall through to the generic acp bridge in
// that case (rare; only happens with future refactors that swap
// the driver type).
func (s *Starter) attachMethodHandler(a *agent.Agent) (*agent.Agent, error) {
	drv, ok := a.Driver().(*acp.DriverHandle)
	if !ok {
		cLog("Start: driver type assert failed; cursor extensions disabled",
			"type", fmt.Sprintf("%T", a.Driver()))
		return a, nil
	}
