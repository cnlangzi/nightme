...
// the runner.
//
// handshakeWindow bounds the wait for AgentConnected; the bridge's
...
//     AgentConnected carrying a non-empty SessionID — so the runtime
...
func TestSession_RealPi_E2E_ReceiveInputAndReply(t *testing.T) {
	...
	// Step 1: handshake → AgentConnected.
	init := mustFirstEventOfKind(t, sess, agent.AgentConnected, handshakeWindow)
