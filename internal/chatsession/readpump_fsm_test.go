...
// the new vocabulary:
//
//   - AgentConnected MUST NOT end the in-flight Prompt — it is session
//     bootstrap metadata (model + session_id), not a turn boundary.
//     In the old model the symptom was "AgentConnected flips the buffer
//     Busy before the first user message is queued"; in the new
//     model the mirror-image hazard is "AgentConnected ends the Prompt
...
// AgentConnected arriving mid-turn must leave the Prompt in flight —
...
func TestEventInit_DoesNotEndPrompt(t *testing.T) {
	...
	case ev := <-as.Events():
		if ev.Kind != KindAgentEvent || ev.AgentEvent == nil ||
			ev.AgentEvent.Kind != agent.AgentConnected {
			t.Fatalf("expected KindAgentEvent{AgentConnected}, got %+v", ev)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("AgentConnected was not drained by the readpump within 2s")
	}

	if !waitReady(as, false, 100*time.Millisecond) {
		t.Fatal("after AgentConnected the AS became ready; AgentConnected must not end the in-flight Prompt")
	}
}
