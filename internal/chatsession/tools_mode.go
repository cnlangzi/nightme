...
//
// ToolsMode controls whether the chat renders tool calls
// (OutToolStart / OutToolEnd) as thread replies under the user
// message, or drops them silently at the outbound EventHandler
// gate.
//
// The merge behaviour (a single thread reply carrying both the
// `● Tool(args)` call line and the `⎿  …` result line via PATCH
// on the same Feishu message_id) is a Channel-internal rendering
// choice — see docs/channel/feishu.md §13.14 and the merge
// helper in internal/channel/feishu/tool_thread_merge.go. The
...
// it, SetToolsMode mutates it, runtime's EventHandler gate
...
// ToolsMode controls per-chat tool-event visibility. The enum
// is ordered such that the zero value is the conservative
// default (tool events hidden); this matters because restored
// ChatSessions whose registry entry lacks the toolsMode field
// (older chat_sessions.json files) decode to ToolsModeHide via
// Go's zero-value semantics. Users who want tool-call
// visibility opt in via /tools on.
//
...
// group chats identically (both /tools off → hide tool events
// in the user-message thread). Mirrors ThinkMode (not
// WatchMode) on this axis.
type ToolsMode int

const (
	// ToolsModeHide is the default: the runtime drops
	// OutToolStart and OutToolEnd events at the EventHandler
	// gate (after Translate + ReplyTo stamping, before ch.Send).
	// The receipt card still carries the final answer; tool
	// activity is just not surfaced in the user-message thread.
	ToolsModeHide ToolsMode = iota

	// ToolsModeShow: the runtime forwards OutToolStart /
	// OutToolEnd to the Channel. Feishu adapter merges each
	// pair into a single thread reply (PATCH on the start
	// message_id when OutToolEnd arrives; falls back to a new
	// thread reply on PATCH failure). The receipt card still
	// carries the final answer as the pinned main-chat view.
	ToolsModeShow
)
...
// a usage hint without committing a state mutation. Mirrors
// ParseThinkMode.
