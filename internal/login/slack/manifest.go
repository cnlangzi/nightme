...
// This matters most on a phone: pasting a 60-line JSON blob into the
...
// AppManifest is the Slack app manifest for nightme.
//
// Pasting this into "Create New App → From an app manifest"
// configures every scope and event subscription in one step, so the
// user never has to hunt through the settings UI.
//
// Scope policy: everything the adapter can use is requested up front
// (decided 2026-08-29). The bot is self-hosted, in the operator's own
// workspace, so there is no third party to protect from the broad
// grant — and a narrow grant would silently break features later,
// after the user has forgotten which checkbox they skipped.
//
...
//
// assistant_view enables the AI-app surface that chat.startStream and
// assistant.threads.setStatus depend on.
const AppManifest = `{
  "_metadata": {
    "major_version": 2,
    "minor_version": 1
