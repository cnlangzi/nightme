...
// proc.New is the bridge spawn recipe in one place. It returns an
...
// codex / opencode / dsh all block on a TTY-driven kqueue if the
// TTY fd is inherited).
//
...
//
// Every bridge spawn site (claudecode / pi / codex / opencode /
// dsh shared-host, plus the PTY path's escape hatch via
// proc.HideWindow) routes through this helper or its sibling
// HideWindow so the platform-specific knob stays in one place;
// any future spawn hardening (e.g. CLOSE_ON_EXEC on inherited
// fds) lands here, not in the per-bridge newSession functions.
