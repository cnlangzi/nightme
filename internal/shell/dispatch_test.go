...
// and verifies the streaming protocol: 1 header + ≥1 chunk +
// 1 footer (= ≥3 sends total), all sharing the same
// ReplyTo (PATCH on the same card), with first = header and
// last = footer.
//
// F-shell-stream design (with debouncer): coalesceLines does
// size-based chunking (4 KiB) and runShell debounces at
// 250ms intervals. For FAST commands (output produced in
// <250ms), all chunks arrive in one debounce window → 1 chunk
// send. For SLOW commands (output spans multiple windows),
// we get multiple chunk sends.
//
// This test asserts the minimum contract (header + ≥1 chunk
// + footer = 3 sends) and the structural invariants
// (first/last markers, ReplyTo consistency, Kind=OutReply).
// It does NOT assert a specific chunk count — that's
// implementation-defined under debouncing.
func TestRunShell_OutReplySequence(t *testing.T) {
	...
	// Fast command: ~25 KB of output in <100ms. With
	// debouncing, this produces 1 chunk send (the whole
	// output fits in one debounce window). The test asserts
	// the minimum shape, not the exact chunk count.
	_, handled := d.Handle(testMgrWithEmitter(t, em), cs, InboundRequest{
	...
	// Wait long enough for the entire shell command to
	// complete + debouncer to flush + footer to send.
	// shellCtx has timeouts.Shell (likely 60s); we wait 5s
	// which is generous for a 5000-line seq.
